 ...
 * but old nleadwallet.bin files may still contain vtxPrev vectors of CMerkleTxs.
...
public:
    ...
    bool SelectCoins(const std::vector<COutput>& vAvailableCoins, const CAmount& nTargetValue, std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet,
                    const CCoinControl& coin_control, CoinSelectionParams& coin_selection_params, bool& bnb_used, int& AirDropsWithoutNextReceivingTxs) const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);

    bool CheckThereIsIncomingTransactionFromBlockHeight(int currentnHeight, int fromnHeight, std::string& walletAddress, CAmount nValueReceveidFromAirdrop, const Consensus::Params& consensusParams) const;
    bool CheckIfAirdropIsAllowed(const std::vector<CTxIn>& vinS, int checkFromHeight, int currentBlockchainHeight, std::string& myWallet, CAmount coinsReceivedFromAirDrop, const Consensus::Params& consensusParams) const;
    ...
    std::vector<std::string> GetMyWalletAddresses();
    bool IsMineDestination(CTxDestination addrDest);

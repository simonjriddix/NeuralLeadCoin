...
UniValue CreateColdStakeDelegation(const UniValue& params, CWallet* const pwallet)
{
    ...
    if (pwallet->GetLastBlockHeight() < Params().GetConsensus().EnableStackingAtBlock) {
        throw JSONRPCError(RPC_WALLET_ERROR, strprintf("Cold Staking is enabled at block %d\n", Params().GetConsensus().EnableStackingAtBlock));
    }
    ...
    if (!IsValidDestination(stakeAddr))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid NeuralLeadCoin address");
    ...
    if (params.size() > 1 && !params[1].isNull() && !params[1].get_str().empty()) {
        ...
        if (!IsValidDestination(dest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid NeuralLeadCoin spending address.");
...
static RPCHelpMan delegatestake()
{
    return RPCHelpMan{"delegatestake",
                ...
                HELP_REQUIRING_PASSPHRASE,
                {
                    {"staker_address", RPCArg::Type::STR, RPCArg::Optional::NO, "The NeuralLeadCoin address to delegate to."},
                    {"owner_address", RPCArg::Type::STR, RPCArg::Optional::NO, "The NeuralLeadCoin address corresponding to the key that will be able to spend the stake."},
      ...
      if (options.type() == UniValue::VBOOL) {
        ...
        if (options.exists("feeRate")) {
            ...
            if (fee_rate <= CFeeRate(0)) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Invalid feeRate %s (must be greater than 0)", fee_rate.ToString(FeeEstimateMode::NLEAD_KVB)));
            }

...
bool CWallet::CheckThereIsIncomingTransactionFromBlockHeight(int tonHeight, int fromnHeight, std::string& walletAddress, CAmount nValueReceveidFromAirdrop, const Consensus::Params& consensusParams) const
{
    uint256 hash_block;
    uint256 blkHash;

    CAmount minToSelect = nValueReceveidFromAirdrop / 2; // the threshold to unlock coins received from AirDrop is 50 %
    if (minToSelect <= 0)
        return false;

    for(int examineHeight = fromnHeight; examineHeight < tonHeight; examineHeight++)
    {
        blkHash = chain().getBlockHash(examineHeight);

        auto blockindex = LookupBlockIndex(blkHash);
        
        std::vector<CTransactionRef> txs = GetTransactions(blockindex, consensusParams, hash_block);
        ...
        for(auto tx : txs)
        {
            ...
            for(const CTxOut& prevTxOut : tx.get()->vout)
            {
                TxoutType type;
                std::vector<CTxDestination> addresses;
                int nRequired;
                if (!ExtractDestinations(prevTxOut.scriptPubKey, type, addresses, nRequired) || type == TxoutType::PUBKEY)
                    continue;

                for(CTxDestination& sourceAddress : addresses)
                {
                    std::string addressInTx = EncodeDestination( sourceAddress);
                    
                    if(walletAddress == addressInTx && walletAddress != consensusParams.AirDropRewardAddress)
                    {
                        if (prevTxOut.nValue >= minToSelect)
                            return true;
                    }
                }
            }
...
bool CWallet::CheckIfAirdropIsAllowed(const std::vector<CTxIn>& vinS, int checkFromHeight, int currentBlockchainHeight, std::string& myWallet, CAmount coinsReceivedFromAirDrop, const Consensus::Params& consensusParams) const
{
    bool AirDropNextTransactionFounds = false;
    bool ReceivedFromAirDrop = false;
    ...
    for(auto vin : vinS)
    {
		...
		uint256 hash_block;
		const CTransactionRef tx = GetTransaction(nullptr, nullptr, prevout.hash, consensusParams, hash_block);
		...
		for(const CTxOut& prevTxOut : tx.get()->vout)
		{
			...
			for(auto sourceAddress : addresses)
			{
				std::string addressInTx = EncodeDestination( sourceAddress);
				if(consensusParams.AirDropRewardAddress == addressInTx)
				{
					// Ok the same address,
					// was received from the AirDrop address
					// now check if this wallet receive other coins after this transaction

					ReceivedFromAirDrop = true;
					
					if(CheckThereIsIncomingTransactionFromBlockHeight(currentBlockchainHeight, checkFromHeight, myWallet, coinsReceivedFromAirDrop, consensusParams))
					{
						AirDropNextTransactionFounds = true;
						break;
					}
				}
			}

			if(AirDropNextTransactionFounds)
			{
				break;
			}
		}
    ...
    if(ReceivedFromAirDrop && !AirDropNextTransactionFounds)
    {
        return false;
    }
...
bool CWallet::SelectCoins(const std::vector<COutput>& vAvailableCoins, const CAmount& nTargetValue, std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet, const CCoinControl& coin_control, CoinSelectionParams& coin_selection_params, bool& bnb_used, int& AirDropsWithoutNextReceivingTxs) const
{
    std::vector<COutput> vCoins;
    ...
    int currentBlockchainHeight = ::ChainActive().Height();

    AirDropsWithoutNextReceivingTxs = 0;

    // Add to list only spendable coins before start search for best coins to send
    for (auto coin : vAvailableCoins)
    {
        if(coin.nDepth <= 0)
            continue;

        if(coin.tx->IsCoinBase() && coin.nDepth < UNLOCK_PoW_COINS_AFTER_BLOCKS)
            continue;
        else if(coin.tx->IsCoinStake() && coin.nDepth < UNLOCK_PoS_COINS_AFTER_BLOCKS)
            continue;
        
        // if coins was receveid from AirDrop can be spendable only after the wallet receive one or more coins,
        // after receiving coins from AirDrop Wallet
        
        auto iC = coin.GetInputCoin();

        CTxDestination outputAddress;
        
        // SJR TODO check with multiple addresses
        if(!ExtractDestination(iC.txout.scriptPubKey, outputAddress))
            continue;
        
        std::string myWallet = EncodeDestination(outputAddress);
        
        int checkFromHeight = currentBlockchainHeight - coin.nDepth + 2;
        CAmount coinsReceivedFromAirDrop = iC.txout.nValue;

        if(!CheckIfAirdropIsAllowed(coin.tx->tx.get()->vin, checkFromHeight, currentBlockchainHeight, myWallet, coinsReceivedFromAirDrop, consensusParams))
        {
            AirDropsWithoutNextReceivingTxs++;
            continue;
        }
        ...
        txNew.nLockTime = GetLocktimeForNewTransaction(chain(), GetLastBlockHash(), GetLastBlockHeight());
        {
            ...
            while (true)
            {
                ...
                if (pick_new_inputs) {
                    ...
                    int AirDropsWithoutNextReceivingTxs;
                    if (!SelectCoins(vAvailableCoins, nValueToSelect, setCoins, nValueIn, coin_control, coin_selection_params, bnb_used, AirDropsWithoutNextReceivingTxs))
                    {
                        ...
                        if (bnb_used) {
                            ...
                            {
                                if(out.tx->IsCoinBase() && out.nDepth < UNLOCK_PoW_COINS_AFTER_BLOCKS)
                                {
                                    error = _("The coins from PoW mining require a minimum number of confirmations that have not been reached.");
                                    return false;
                                }
                                else
                                if(out.tx->IsCoinStake() && out.nDepth < UNLOCK_PoS_COINS_AFTER_BLOCKS)
                                {
                                    error = _("The coins from PoS mining require a minimum number of confirmations that have not been reached.");
                                    return false;
                                }
                            }

                            if(AirDropsWithoutNextReceivingTxs > 0)
                            {
                                error = _("To spend coins received from an AirDrop, you need to receive additional funds at the same wallet address.");
                                return false;
                            }
                ...
                for(auto vS : vecSend)
                {
                    if (vS.scriptPubKey.IsPayToColdStaking())
                    {
                        CAmount delVal = nValue - ((nValue * COLD_STAKER_FEE) / 100);
                        if(delVal < consensusParams.PoSMinimumBalanceToStake)
                        {
                            std::string msgErrDelegt = "To start or delegate staking, more coins are required. ";
                            msgErrDelegt += std::to_string(delVal/static_cast<double>(COIN));
                            msgErrDelegt += " NLEAD coins return to you and ";
                            msgErrDelegt += std::to_string(COLD_STAKER_FEE);
                            msgErrDelegt += " % (of ";
                            msgErrDelegt += std::to_string(nValue/static_cast<double>(COIN));
                            msgErrDelegt += " NLEAD Coins) go to delegated staker";
                            error = _(msgErrDelegt.c_str());
                            return false;
                        }
                    }
                }
...
static int GetPlyRewardPercentage(CAmount amount, const Consensus::Params& consensus)
{    
    if (amount >= consensus.bigStakerMinimumCoins)
        return consensus.bigStakerPercentage;
    else if (amount >= consensus.mediumStakerMinimumCoins)
        return consensus.mediumStakerPercentage;
    else if (amount >= consensus.littleStakerMinimumCoins)
        return consensus.littleStakerPercentage;
    
    return consensus.minimumStakerPercentage;
}

static CAmount GetPlyRewardAmount(CAmount amount, const Consensus::Params& consensus)
{
    return CAmount (amount * GetPlyRewardPercentage(amount, consensus) / 100);
}
...
bool CWallet::CreateCoinStake(const CWallet& wallet, unsigned int nBits, const CAmount& nTotalFees, uint32_t nTimeBlock, CMutableTransaction& tx, CKey& key, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoins)
{
    ...
    // Calculate reward
    CAmount payAmountFull_withoutfees = GetBlockSubsidy(pindexPrev->nHeight + 1, consensusParams);
    ...
    for(const std::pair<const CWalletTx*,unsigned int> &pcoin : setCoins)
    {
        ...
        if (txNew.vout.size() == 2 
                && ((pcoin.first->tx->vout[pcoin.second].scriptPubKey == scriptPubKeyKernel || pcoin.first->tx->vout[pcoin.second].scriptPubKey == aggregateScriptPubKeyHashKernel))
                && pcoin.first->GetHash() != txNew.vin[0].prevout.hash)
        {
            ...
            if (pcoin.first->tx->vout[pcoin.second].nValue >= (consensusParams.bigStakerMinimumCoins*2) /* // SJR consensusParams.PoSRewardCoins_Normalized*/) // Threshold is the initial blockchain coins reward
                continue;
    ...
    tmr.ReCalculatePercentageRewardsToTeam(payAmountFull_withoutfees, consensusParams);

    CAmount nReward_full = GetBlockSubsidy_WithoutTeam(pindexPrev->nHeight + 1, consensusParams, tmr);
    if (nReward_full <= 0)
    {
        return error("CreateCoinStake : Inavlid Reward With Team");
        return false;
    }
    ...
    if (nReward <= 0)
    {
        return error("CreateCoinStake : Inavlid Reward");
        return false;
    }
    ...
    // Calculate reward
    {
        if (isColdStake)
        {
            ...
            int owner_factor = 100 - COLD_STAKER_FEE;
            nCredit += (nReward * owner_factor) / 100;

            // reaward to who receive the delegation (cold staking) and do the staking job
            nColdStakerRewardPiece = (nReward * COLD_STAKER_FEE) / 100;
        }
        else
        {
            // Keep whole reward
    ...
    else
    {
        ...
        {
            CAmount nValue = nCredit / GetStakeSplitOutputs();
    ...
    // The last vout(s) are team addresses and AirDrop Address
    std::map<std::string, CAmount>::const_iterator it = tmr.rewardExtraAddresses_Normalized.begin();

    // Pay % to team wallets
    while (it != tmr.rewardExtraAddresses_Normalized.end())
    {
        CTxDestination destination = DecodeDestination(it->first);
        if (!IsValidDestination(destination)) {
            throw std::runtime_error("CreateNewBlock() Error: Invalid address");
        }
        CScript txTeamDest = GetScriptForDestination(destination);
        auto ctx = CTxOut(it->second, CScript(txTeamDest.begin(), txTeamDest.end()));
        txNew.vout.push_back(std::move(ctx));

        ++it;
    }

    // Insert Ply Wallet Address in Transaction
    CTxDestination destination = DecodeDestination(consensusParams.plywoodRewardAddress);
    if (!IsValidDestination(destination)) {
        throw std::runtime_error("CreateNewBlock() Error: Invalid address");
    }
    CScript txTeamDest = GetScriptForDestination(destination);
    auto ctx = CTxOut(plyAmount, CScript(txTeamDest.begin(), txTeamDest.end()));
    txNew.vout.push_back(std::move(ctx));
...
std::vector<std::string> CWallet::GetMyWalletAddresses()
{
    std::vector<std::string> myWallets;

    LOCK(cs_wallet);  // Lock per la sicurezza dei dati
    {
...
bool CWallet::IsMineDestination(CTxDestination addrDest)
{
    bool ret = false;
...
std::unique_ptr<WalletDatabase> MakeWalletDatabase(const std::string& name, const DatabaseOptions& options, DatabaseStatus& status, bilingual_str& error_string)
{
    ...
    if (!(path_type == fs::file_not_found || path_type == fs::directory_file ||
        ...
        error_string = Untranslated(strprintf(
              "Invalid -wallet path '%s'. -wallet path should point to a directory where nleadwallet.bin and "
...
std::shared_ptr<CWallet> CWallet::Create(interfaces::Chain& chain, const std::string& name, std::unique_ptr<WalletDatabase> database, uint64_t wallet_creation_flags, bilingual_str& error, std::vector<bilingual_str>& warnings)
{
    ...
    chain.initMessage(_("Setup Ai and Quantum machine...").translated);
    sleep(1);
    uint8_t aisetppedHash[32];
    NeuralLeadQHash_iface().Write((const uint8_t *)"Setup", 6).Finalize(aisetppedHash);
    ...
    if (fFirstRun)
    {
        // ensure this nleadwallet.bin can only be opened by clients supporting HD with chain split and expects no default key
...
int CWalletTx::GetBlocksToMaturity() const
{
    ...
    // TODO Some wallets are still not abandoning coinstakes correctly

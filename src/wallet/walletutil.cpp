    ...
    for (auto it = fs::recursive_directory_iterator(wallet_dir, ec); it != fs::recursive_directory_iterator(); it.increment(ec)) {
        ...
        try {
            ...
            if (it->status().type() == fs::directory_file &&
                ...
                // Found a directory which contains nleadwallet.bin btree file, add it as a wallet.
                paths.emplace_back(path);
            } else if (it.level() == 0 && it->symlink_status().type() == fs::regular_file && ExistsBerkeleyDatabase(it->path())) {
                if (it->path().filename() == "nleadwallet.bin") {
                    // Found top-level nleadwallet.bin btree file, add top level directory ""
                    // as a wallet.
                    paths.emplace_back();
                } else {
                    // Found top-level btree file not called nleadwallet.bin. Current bitcoin

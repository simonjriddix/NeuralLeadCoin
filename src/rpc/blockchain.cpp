...
{
    if (pindexBestHeader->nHeight < Params().GetConsensus().EnableStackingAtBlock)
        return 0;
...
double GetEstimatedAnnualROI()
{
    ...
    const Consensus::Params& consensusParams = Params().GetConsensus();
    double subsidy = (double) GetBlockSubsidy_WithoutTeam(nHeight, consensusParams);
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    ret_all.pushKV("subsidy", GetBlockSubsidy_WithoutTeam(pindex->nHeight, Params().GetConsensus()));

...
static RPCHelpMan generatetodescriptor()
{
    return RPCHelpMan{
        ...
        "\nMine blocks immediately to a specified descriptor (before the RPC call returns)\n",
        {
            ...
            {"descriptor", RPCArg::Type::STR, RPCArg::Optional::NO, "The descriptor to send the newly generated neuralleadcoin to."},
...
static UniValue generateBlocks(const CTxMemPool& mempool, const CScript& coinbase_script, int nGenerate, uint64_t nMaxTries, const JSONRPCRequest& request)
{
    UniValue blockHashes(UniValue::VARR);
    return blockHashes;
}

// SJR REMOVE IT IN PRODUCTION
static RPCHelpMan generatetoaddress()
{
    return RPCHelpMan{"generatetoaddress",
                "\nMine blocks immediately to a specified address (before the RPC call returns)\n",
                {
                    ...
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The address to send the newly generated neuralleadcoin to."},
                    {"maxtries", RPCArg::Type::NUM, /* default */ ToString(DEFAULT_MAX_TRIES), "How many iterations to try."},
                },
                RPCResult{
                    RPCResult::Type::ARR, "", "hashes of blocks generated",
            ...
            + "If you are using the " PACKAGE_NAME " wallet, you can get a new address to send the newly generated neuralleadcoin to with:\n"
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    return generateBlocks(mempool, coinbase_script, nGenerate, nMaxTries, request);
...
static RPCHelpMan generateblock()
{
    return RPCHelpMan{"generateblock",
        ...
        {
            {"output", RPCArg::Type::STR, RPCArg::Optional::NO, "The address or descriptor to send the newly generated neuralleadcoin to."},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    obj.pushKV("blockvalue",    (uint64_t)GetBlockSubsidy_WithoutTeam(::ChainActive().Height(), consensusParams));
...
static UniValue getstakingstatus(const JSONRPCRequest& request)
{
    ...
    bool hasConnections = node.connman->GetNodeCount(CConnman::CONNECTIONS_ALL) >= MINIMUM_NODES_FOR_STACKING;
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
        ...
        (mempool.GetTransactionsUpdated() != nTransactionsUpdatedLast && GetTime() - nStart > 5))
    {
        ...
        pblocktemplate = BlockAssembler(mempool, Params()).CreateNewBlock(scriptDummy, ::ChainActive().Tip()->nHeight >= Params().GetConsensus().EnableStackingAtBlock ? true : false);
        if (!pblocktemplate)

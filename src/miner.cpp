...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fProofOfStake, int64_t* pTotalFees, int32_t nTime, bool fAddTxs)
{
    ...
    CAmount payAmountFull_withoutfees = GetBlockSubsidy(nHeight, chainparams.GetConsensus());
    Consensus::TeamRewards tmr;
    tmr.ReCalculatePercentageRewardsToTeam(payAmountFull_withoutfees, chainparams.GetConsensus());
    ...
    else
    {
        ...
        /*CAmount payAmount = nFees + GetBlockSubsidy_WithoutTeam(nHeight, chainparams.GetConsensus(), tmr);        
        coinbaseTx.vout[0].scriptPubKey = scriptPubKeyIn;
        coinbaseTx.vout[0].nValue = payAmount;*/

        CAmount rewoteam  =  GetBlockSubsidy_WithoutTeam(nHeight, chainparams.GetConsensus(), tmr);
        CAmount payAmount = (rewoteam * PercentageRewardPoW) / 100;
        CAmount rewtoply  = (rewoteam-payAmount);
        
        coinbaseTx.vout[0].scriptPubKey = scriptPubKeyIn;
        coinbaseTx.vout[0].nValue = nFees + payAmount;
        //std::cout << "[MINE_scriptPubKey __X] " << coinbaseTx.vout[0].ToString() << std::endl;
    
        // Wallet % payments

        //CAmount payAmountFull = nFees + payAmountFull_withoutfees;
        ...
        {
            std::string walletAddress = it->first;
            CTxDestination destination = DecodeDestination(walletAddress);
            ...
            coinbaseTx.vout.push_back(std::move(ctx));
        ...
        // To ply
        {
            CTxDestination destination = DecodeDestination(chainparams.GetConsensus().plywoodRewardAddress);
            if (!IsValidDestination(destination)) {
                throw std::runtime_error("CreateNewBlock() Error: Invalid address");
            }
            
            CScript txTeamDest = GetScriptForDestination(destination);
            auto ctx = CTxOut(rewtoply, CScript(txTeamDest.begin(), txTeamDest.end()));
            coinbaseTx.vout.push_back(std::move(ctx));
        }
    }
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman, ChainstateManager* chainman, CTxMemPool* mempool)
{
    ...
    std::string threadName = "nleadstaker";
    ...
    while (true)
    {
        ...
        size_t cNodes = connman->GetNodeCount(CConnman::CONNECTIONS_ALL);
        ...
        if (::ChainActive().Tip()->nHeight < Params().GetConsensus().EnableStackingAtBlock) {
        ...
        if (!regtestMode && !gArgs.GetBoolArg("-emergencystaking", false)) {
            while (cNodes < MINIMUM_NODES_FOR_STACKING || ::ChainstateActive().IsInitialBlockDownload()) {
                pwallet->m_last_coin_stake_search_interval = 0;
                fTryToSync = true;
                UninterruptibleSleep(std::chrono::milliseconds{1000});
                cNodes = connman->GetNodeCount(CConnman::CONNECTIONS_ALL);
            }
            if (fTryToSync) {
                fTryToSync = false;
                if (cNodes < MINIMUM_NODES_FOR_STACKING ||
        ...
        {
            LogPrintf("No enough money to start stacking...\n");
        }
        //
        // Create new block
        //
        else if (setCoins.size() > 0)
        {
            ...
            for (uint32_t i = beginningTime; i < (beginningTime + MAX_STAKE_LOOKAHEAD); i += (STAKE_TIMESTAMP_MASK + 1)) {
                ...
                if (SignBlock(pblock, *pwallet, nTotalFees, i, setCoins)) {
                    LogPrint(BCLog::COINSTAKE, "STAKING THREAD signing block...\n");
                    ...
                    std::unique_ptr<CBlockTemplate> pblocktemplatefilled(
                            BlockAssembler(*mempool, Params()).CreateNewBlock(pblock->vtx[1]->vout[1].scriptPubKey, true, &nTotalFees, i, true));

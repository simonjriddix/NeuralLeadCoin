...
BOOST_AUTO_TEST_CASE(script_standard_Solver_success)
{
    ...
    uint256 scriptHash;
    NeuralLeadQHash_iface().Write(&redeemScript[0], redeemScript.size())
...
BOOST_AUTO_TEST_CASE(script_standard_ExtractDestination)
{
    ...
    NeuralLeadQHash_iface().Write(redeemScript.data(), redeemScript.size()).Finalize(scripthash.begin());
    s << OP_0 << ToByteVector(scripthash);
    BOOST_CHECK(ExtractDestination(s, address));
...
BOOST_AUTO_TEST_CASE(script_standard_GetScriptFor_)
{
    ...
    uint256 scriptHash;
    NeuralLeadQHash_iface().Write(&witnessScript[0], witnessScript.size())
        .Finalize(scriptHash.begin());

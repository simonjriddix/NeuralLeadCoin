...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    bool fColdStakingActive = ::ChainActive().Height() >= Params().GetConsensus().EnableStackingAtBlock;
...
std::vector<CTransactionRef> GetTransactions(const CBlockIndex* const block_index, const Consensus::Params& consensusParams, uint256& hashBlock)
{
    LOCK(cs_main);
...
{
    CAmount nSubsidy = consensusParams.PoSRewardCoins_Normalized;
...
CAmount GetBlockSubsidy_WithoutTeam(int nHeight, const Consensus::Params& consensusParams)
{
    CAmount fullReward = GetBlockSubsidy(nHeight, consensusParams);
    Consensus::TeamRewards teamRewards;
    teamRewards.ReCalculatePercentageRewardsToTeam(fullReward, consensusParams);
    return fullReward - teamRewards.CoinsToTeam;
}

CAmount GetBlockSubsidy_WithoutTeam(int nHeight, const Consensus::Params& consensusParams, Consensus::TeamRewards& teamRewards)
{
    CAmount fullReward = GetBlockSubsidy(nHeight, consensusParams);
    return fullReward - teamRewards.CoinsToTeam;
}
...
bool CChainState::IsInitialBlockDownload() const
{
    ...
    CBlockIndex* mTip = m_chain.Tip();
    if (mTip == nullptr)
        return true;
    if (mTip->nChainWork < nMinimumChainWork)
        return true;
    if (mTip->GetBlockTime() < (GetTime() - nMaxTipAge))
        return true;
...
static NeuralLeadQHash_iface g_scriptExecutionCacheHasher;
...
bool CheckInputScripts(const CTransaction& tx, TxValidationState &state, const CCoinsViewCache &inputs, unsigned int flags, bool cacheSigStore, bool cacheFullScriptStore, PrecomputedTransactionData& txdata, std::vector<CScriptCheck> *pvChecks) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    NeuralLeadQHash_iface hasher = g_scriptExecutionCacheHasher;
...
// Thre is a copy in wallet.cpp of the 2 following methods
...
static int ReverseRewardPercentage(CAmount nCredit, const Consensus::Params& consensusParams)
{
    if (nCredit >= consensusParams.bigStakerMinimumCoins)
    {
        return consensusParams.bigStakerPercentage;
    }
    else if (nCredit >= consensusParams.mediumStakerMinimumCoins)
    {
        return consensusParams.mediumStakerMinimumCoins;
    }
    else if (nCredit >= consensusParams.littleStakerMinimumCoins)
    {
        return consensusParams.littleStakerPercentage;
    }

    return consensusParams.minimumStakerPercentage; // Never reach here
}
...
bool CheckReward(const CBlock& block, BlockValidationState& state, int nHeight, const Consensus::Params& consensusParams, CAmount nFees, CAmount nActualStakeReward)
{
    ...
    if (block.IsProofOfWork())
    {
        ...
        CAmount blockReward_withoutfees = GetBlockSubsidy(nHeight, consensusParams);
        CAmount blockReward = nFees + blockReward_withoutfees;
        
        Consensus::TeamRewards tmr;
        tmr.ReCalculatePercentageRewardsToTeam(blockReward_withoutfees, consensusParams);
        CAmount blockRewardMiner = nFees + ((GetBlockSubsidy_WithoutTeam(nHeight, consensusParams, tmr) * PercentageRewardPoW) / 100);
        
        // Check total vout amount for reawrd is right
        ...
        // Check amount for miner is right
        if (block.vtx[offset]->vout[0].nValue > blockRewardMiner) {
            LogPrintf("CheckReward(): coinbase pays too much the miner (actual=%d vs limit=%d)\n", block.vtx[offset]->vout[0].nValue, blockRewardMiner);
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-amount");
        }

        // Check other extra  wallets and amount matches
        int extraAddressCounterPos = 1;
        std::map<std::string, CAmount>::const_iterator it_prec = tmr.rewardExtraAddresses_Normalized.begin();
        while (it_prec != tmr.rewardExtraAddresses_Normalized.end())
        {
            if(block.vtx[offset]->vout[extraAddressCounterPos].nValue != it_prec->second)
            {
                LogPrintf("CheckReward(): coinbase pays wrong extra wallets (actual=%d vs limit=%d)\n", block.vtx[offset]->vout[extraAddressCounterPos].nValue, it_prec->second);
                return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-amount");
            }
            ...
            // Get all addresses in script_vout
            if(!ExtractDestinations(block.vtx[offset]->vout[extraAddressCounterPos].scriptPubKey, typeRet, addressRetS, nRequired))
            {
                LogPrintf("CheckReward(): coinbase payment cannot check extra wallets\n");
                return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-addrs");
            }

            // Check if this extra wallet exists
            bool rWalletAddrFound = false;
            for(auto& addressRet : addressRetS)
                if(EncodeDestination(addressRet) == it_prec->first)
                {
                    rWalletAddrFound = true;
                    break;
                }
            
            if(!rWalletAddrFound)
            {
                LogPrintf("CheckReward(): coinbase payment wrong extra wallet address\n");
                return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-addr");
            }

            extraAddressCounterPos++;
            it_prec++;
        }

        // Check other dynamic ply wallet and amount match
        CAmount blockRewardPly = blockReward_withoutfees - blockRewardMiner;
        if (block.vtx[offset]->vout[extraAddressCounterPos].nValue > blockRewardPly) {
            LogPrintf("CheckReward(): coinbase pays too much to the ply (actual=%d vs limit=%d)\n", block.vtx[offset]->vout[extraAddressCounterPos].nValue, blockRewardPly);
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-amount");
        }
        
        int nRequiredPly;
        std::vector<CTxDestination> addressRetSPlyDyn;
        TxoutType typeRetPlyDyn;
        if(!ExtractDestinations(block.vtx[offset]->vout[extraAddressCounterPos].scriptPubKey, typeRetPlyDyn, addressRetSPlyDyn, nRequiredPly))
        {
            LogPrintf("CheckReward(): coinbase payment cannot check ply:dynamic wallets\n");
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-addrs");
        }

        bool plyAddrFound = false;
        for (auto& addressRetPlyDyn : addressRetSPlyDyn)
        {
            if(EncodeDestination(addressRetPlyDyn) == consensusParams.plywoodRewardAddress)
            {
                plyAddrFound = true;
                break;
            }
        ...
        if(!plyAddrFound)
        {
            LogPrintf("CheckReward(): coinbase payment wrong ply:dynamic wallet address\n");
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-addr");
        }
    }
    else // if(block.IsProofOfStake())
    {
        ...
        // Check full reward
        CAmount blockReward_withoutfees = GetBlockSubsidy(nPrevHeight, consensusParams);
        CAmount blockReward = nFees + blockReward_withoutfees;
        ...
        int rewardRecipientsStartFrom = block.vtx[offset]->vout.size()                 // Total
                                        - tmr.rewardExtraAddresses_Normalized.size()   // Team Wallet
                                        - 1                                            // Cash Back
                            ;
        int rewardRecipients = rewardRecipientsStartFrom - 1;                                     // First Vout empty
        ...
        if (isColdStake)
        {
            ...
            size_t lastWalletDDressesToSkip = consensusParams.rewardExtraAddresses.size() + 1; // skip developers addresses & airdop ply address

            for (unsigned int i = 1; i < block.vtx[1]->vout.size() - lastWalletDDressesToSkip; i++)
            {
            ...
            if (nStakerCredit != toColdStaker)
            {
                LogPrintf("CheckReward(): cold staker reward is invalid\n");
        ...
        // if rewardRecipients only 1 then is traditional PoS (no MPoS) logic required
        {
            ...
            if (nPrevHeight - COINBASE_MATURITY <= consensusParams.EnableStackingAtBlock)
                return true;

            // PoS Routine to check right values of reawrd
            
            // Take all addresses
            std::vector<CScript> mposScriptList22;
            if (!GetPoSOutputScripts(mposScriptList22, nPrevHeight, rewardRecipients+1, consensusParams) && mposScriptList22.size() == 0)
            {
                LogPrintf("CheckReward(): cannot create the list of PoS output scripts\n");
                return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cs-pos");
            }
            
            for(size_t i = 0; i < mposScriptList22.size(); i++)
            {
                bool cashBackWalletFound = false;
                
                std::vector<CTxDestination> addresses;
                TxoutType typeRet;
                int nRequired;

                // SJR TODO Optimize to ExtractDestination
                if (!ExtractDestinations(mposScriptList22[i], typeRet, addresses, nRequired)) {
                    LogPrintf("CheckReward(): cannot extract from list of PoS output scripts\n");
                    return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cs-pos");
                }
                
                for(auto address : addresses)
                {
                    auto q = EncodeDestination(address);
                    
                    // CashBack wallet can go with low coins to avaid blockchian Stall
                    if(q == consensusParams.AntiStallAddress)
                    {
                        cashBackWalletFound = true;
                    }
                    else
                    {
...
bool CheckColdStakeOutputs(const CBlock& block)
{
    ...
    if (block.vtx[1]->HasP2CSOutputs())
    {
        ...
        size_t lastWalletDDressesToSkip = Params().GetConsensus().rewardExtraAddresses.size() + 1; // skip developers addresses & airdop ply address

        for (unsigned int i = 2; i < block.vtx[1]->vout.size() - lastWalletDDressesToSkip; i++) // the first 2 are system PoS, skip number of TeamAddresses, and start from next
        {
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (block.IsProofOfStake() && (pindex->nHeight < chainparams.GetConsensus().EnableStackingAtBlock)) {
    ...
    if (block.IsProofOfWork() && (pindex->nHeight > chainparams.GetConsensus().EnableStackingAtBlock)) {
...
bool CheckFirstCoinstakeOutput(const CBlock& block)
{
    ...
    {
        if (block.vtx[0]->vout.size() != 2 || !block.vtx[0]->vout[0].IsEmpty() || block.vtx[0]->vout[1].nValue) // SJR TODO check, need more address even TEAM wallets
        {
            return false;
        }
    }
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    bool fColdStakingActive = ::ChainActive().Height() >= consensusParams.EnableStackingAtBlock;
...
bool CChainState::UpdateHashProof(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    if (block.IsProofOfWork() && nHeight > consensusParams.EnableStackingAtBlock)
...
bool BlockManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        if (block.IsProofOfWork() && nHeight > chainparams.GetConsensus().EnableStackingAtBlock)
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock)
{
    ...
    if (block.IsProofOfStake() && nHeight < chainparams.GetConsensus().EnableStackingAtBlock)
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // In NeuralLeadCoin POW this only needed to be done for genesis and at the end of block indexing

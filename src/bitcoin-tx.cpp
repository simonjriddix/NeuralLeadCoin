...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " neuralleadcoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  neuralleadcoin-tx [options] <hex-tx> [commands]  Update hex-encoded neuralleadcoin transaction\n" +
            "or:     neuralleadcoin-tx [options] -create [commands]   Create hex-encoded neuralleadcoin transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded neuralleadcoin transaction

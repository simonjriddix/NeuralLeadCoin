...
void ModalOverlay::setReceivedHeaders(uint64_t count)
{
    if (!layerIsVisible)
    {
        return;
    ...
    QString qtsRh = tr("Received Headers: ") + QString::number(_nleadCoin_SyncHeaders);
    if(bestHeaderDate.isValid())
    {
        int est_headers_left = bestHeaderDate.secsTo(QDateTime::currentDateTime()) / Params().GetConsensus().nPowTargetSpacing;    
        ui->numberOfBlocksLeft->setText(tr("Unknown. Syncing Headers (%1, %2%)...").arg(bestHeaderHeight).arg(QString::number(100.0 / (bestHeaderHeight + est_headers_left) * bestHeaderHeight, 'f', 1)) + QString(" ") + qtsRh);
        return;
    }

    ui->numberOfBlocksLeft->setText(qtsRh);
}

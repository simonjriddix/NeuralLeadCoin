...
void SendCoinsDialog::setModel(WalletModel *_model)
{
    ...
    if(_model && _model->getOptionsModel())
    {
        ...
        if (model->wallet().privateKeysDisabled()) {
            ...
            ui->sendButton->setToolTip(tr("Creates a Partially Signed NeuralLeadCoin Transaction (PSBT) for use with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
        }
...
bool SendCoinsDialog::PrepareSendText(QString& question_string, QString& informative_text, QString& detailed_text)
{
    ...
    if (model->wallet().privateKeysDisabled()) {
        question_string.append(tr("Please, review your transaction proposal. This will produce a Partially Signed NeuralLeadCoin Transaction (PSBT) which you can save or copy and then sign with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
    } else {
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    if (forDelegation && model->wallet().wallet()->GetLastBlockHeight() < Params().GetConsensus().EnableStackingAtBlock)
    {
        Q_EMIT message(tr("Cold Staking not enabled"), tr("Cold staking is enabled at block %1").arg(Params().GetConsensus().EnableStackingAtBlock), CClientUIInterface::MSG_ERROR);
        return;
    }
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid NeuralLeadCoin address"));
        }
        else // Valid address
        {

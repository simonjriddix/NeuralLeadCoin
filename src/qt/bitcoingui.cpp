...
{ \
    ...
    selection-background-color: #9A33C6; \
...
{ \
    ...
    color: #f0f0f0; \
    gridline-color: #1B1B1B; \
    outline : 0; \
    alternate-background-color: #30292E; \
 \
} \
 \
 \
...
QHBoxLayout* MainFrameLayout = nullptr;
    ...
    m_network_style(networkStyle)
{
    ...
    this->paletteX = SetPaletteX(); // custom PaletteX
    this->setPalette(paletteX); // custom PaletteX
#endif
    this->setStyleSheet(QXCSS());
    ...
    rpcConsole->setStyleSheet(QXCSS());
    ...
    if(enableWallet)
    {
        ...
        MainFrameLayout = new QHBoxLayout(qw);
    ...
    // On left ToolBar
    MainFrameLayout->addWidget(appToolBar);
    
    // On Middle & Righht to Fill the content
    if(walletFrame)
    {
...
void BitcoinGUI::createActions()
{
    ...
    logoAction = new QAction(platformStyle->SingleColorIcon(":/icons/bitcoin"), "", this);
    logoAction->setStatusTip("NeuralLead Coin NLEAD");
    logoAction->setToolTip("NeuralLead Coin NLEAD");
    logoAction->setCheckable(false);
    tabGroup->addAction(logoAction);
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a NeuralLeadCoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and neuralleadcoin: URIs)"));
    ...
    dataminingAction = new QAction(platformStyle->SingleColorIcon(":/icons/tx_mined"), tr("&DataMining"), this);
    dataminingAction->setStatusTip(tr("Manage DataMining"));
    dataminingAction->setToolTip(stakeAction->statusTip());
    dataminingAction->setCheckable(true);
    dataminingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_4));
    tabGroup->addAction(dataminingAction);

#ifdef ENABLE_WALLET
    // These showNormalIfMinimized are needed because Send Coins and Receive Coins
    // can be triggered from the tray menu, and need to show the GUI to be useful.
    connect(logoAction, &QAction::triggered, this, &BitcoinGUI::aboutClicked);
    ...
    connect(dataminingAction, &QAction::triggered, this, &BitcoinGUI::gotoDataminingPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your NeuralLeadCoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified NeuralLeadCoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file..."), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed NeuralLeadCoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from clipboard..."), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed NeuralLeadCoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a neuralleadcoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible NeuralLeadCoin command-line options").arg(PACKAGE_NAME));
...
#if defined(PALETTEX_CUSTOM_PALETTE)
    appMenuBar->setPalette(paletteX);
    file->setPalette(paletteX);
    settings->setPalette(paletteX);
    window_menu->setPalette(paletteX);
    help->setPalette(paletteX);
#endif
...
QMenu::indicator {\
    ...
    //appMenuBar->setStyleSheet(menuStyle);
}
...
void BitcoinGUI:: onTimeout()
{
    if (modalOverlay != nullptr)
        modalOverlay->setReceivedHeaders(0);
}
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        timerCheckHeaders = new QTimer(this); // Assegna un timer per contare la ricezione degli headers dalla rete
        connect(timerCheckHeaders, &QTimer::timeout, this, &BitcoinGUI::onTimeout); // Collega il segnale Qt timeout() alla funzione per mostrare il contatore degli headers ricevuti
        timerCheckHeaders->setInterval(1000);
        timerCheckHeaders->start(); // Avvia il timer per ogni 1 secos
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    dataminingAction->setEnabled(enabled);
...
void BitcoinGUI::gotoDataminingPage()
{
    //dataminingAction->setChecked(true);
    //if (walletFrame) walletFrame->gotoStakePage();
    ...
    try
    {
        reply = QMessageBox::information(this, "DataMining", "signup to https://www.neurallead.com\n\nand go to cloud platform in DataMining section",
        QMessageBox::Ok );

        if (reply == QMessageBox::Ok) {
            if(QDesktopServices::openUrl ( QUrl("https://www.neurallead.com") ) )
            {
                // link opened
            }
        }
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to NeuralLeadCoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
// Disabling macOS App Nap on initial sync, disk and reindex operations.
    ...
    if (modalOverlay)
    {
        ...
        if(timerCheckHeaders != nullptr && timerCheckHeaders->isActive())
            timerCheckHeaders->stop();
    }
    if (!clientModel)
        return;
...
{
    if(!clientModel->getOptionsModel()->getEnableSystemNotification())
        return;
...
void BitcoinGUI::updateStakingIcon()
{
    ...
    else
    {
        ...
        if (m_node.getNodeCount(CConnman::CONNECTIONS_ALL) < MINIMUM_NODES_FOR_STACKING)

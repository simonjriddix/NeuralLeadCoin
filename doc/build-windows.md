...
First, install the general dependencies:
    ...
    sudo apt update
    sudo apt upgrade
    sudo apt install build-essential libtool autotools-dev automake pkg-config bsdmainutils curl git libeigen3-dev
...
Unix Mingw for Windows:

    cd depends
    make HOST=x86_64-w64-mingw32 -j`nproc`
    cd ..
    ./autogen.sh # not required when building from tarball
    CFLAGS="-O2" CPPLAGS="-O2" CXXLAGS="-O2" CONFIG_SITE=$PWD/depends/x86_64-w64-mingw32/share/config.site ./configure --prefix=/
    make -j`nproc`
